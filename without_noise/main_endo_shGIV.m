...
for i=1:size(t_tG_shGIV,1)
    ...
    [h,p]=ttest2(Y_tG(ind1,:),Y_tG_shGIV(i,:),'Tail','Right');  [h p]
end

%% simulation --------------------------------
Input_all=[0 0.23]; % stimulus =0 for simulating the serum-starved condition, and 0.23 for EGF=50nM.
tau(1)=0.25;tau(2)=15;tau(3)=0.35;tau(4)=0.1;
tau(5)=15; tau(6)=1; tau(7)=1000;   %time scales for mGEF, mGAP, mG, tGEF,tG ,tGAP,secretion;
...
open('tg.fig');hold on;
plot(TT,Y(:,5)/tg_wild,'r');save('Fig2D.eps');
